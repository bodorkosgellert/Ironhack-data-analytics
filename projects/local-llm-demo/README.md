...
Local inference is still a relevant pattern when the corpus would be denser and more personal: clinic notes, symptom diaries, wearable streams, home spirometry, medication logs, geolocation traces, or other Institutional Review Board (IRB)–bound extracts that must not leave an institutional network. In those settings, benefits of a local (or privately hosted) retrieval-plus-generation design include data residency, avoiding third-party training exposure for prompts and passages, auditability of retrieval and prompts, and pairing generation with deterministic metric routing so stored numbers remain authoritative. Speed on a laptop is a secondary benefit; **data density and sensitivity** are the primary justification for keeping generation on-device or inside a private network.

### Literature examples of denser, person-linked asthma corpora

These publications illustrate study designs that collect **individual-level or continuously tracked** asthma-related data (surveys, devices, location, air exposure). They are **motivation for the architecture pattern**, not sources indexed by this demo:

1. **Asthma Mobile Health Study (ResearchKit)** — Chan et al., *Nature Biotechnology* (2017): large remote iPhone study with surveys, devices, geolocation, and air-quality linkage; authors explicitly discuss data security among the challenges. PubMed: https://pubmed.ncbi.nlm.nih.gov/28288104/
2. **Asthma Mobile Health Study data descriptor** — Chan et al., *Scientific Data* (2018): describes smartphone-collected resources (thousands of U.S. participants who agreed to broad sharing) and notes passive location tracking among the capabilities that create deep individual data. DOI: https://doi.org/10.1038/sdata.2018.96
3. **Asthma Health app → EHR integration (patient-generated health data)** — case report on sharing inhaler use and peak flow from a smartphone asthma app into an electronic health record for clinical review (NPJ Digital Medicine, 2018). DOI: https://doi.org/10.1038/s41746-018-0030-8
4. **AirPredict** — eHealth platform combining wearable air-quality sensing, digital asthma diaries, portable spirometry, and related signals; authors describe GDPR-oriented controls (for example ZIP-level location rather than exact GPS, encryption, pseudonymisation). Frontiers in Digital Health (2025): https://www.frontiersin.org/journals/digital-health/articles/10.3389/fdgth.2025.1573342/full
5. **Digital health ecosystem for asthma and air quality (early feasibility)** — patient-reported and clinician-facing monitoring with pseudonymised uploads and controlled clinical access. Frontiers in Digital Health (2026): https://www.frontiersin.org/journals/digital-health/articles/10.3389/fdgth.2026.1766023/full

This list is a **sample of clearly relevant examples**, not a complete count of all asthma studies worldwide. Many additional cohorts (clinic registries, trial datasets, other mHealth apps) follow the same privacy-sensitive pattern: once records identify or closely track people over time, sending raw passages to a public hosted chatbot is usually the wrong default.

Limits remain: a local large language model is not Health Insurance Portability and Accountability Act (HIPAA) or General Data Protection Regulation (GDPR) compliance by itself. Access control, encryption, logging, retention policy, and institutional governance still apply. Any future private corpus would need its own allowlist, evaluation, and legal review; this public demo does not substitute for that work.
