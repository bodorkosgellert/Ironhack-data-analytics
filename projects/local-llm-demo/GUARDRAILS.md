...
| **L3** | Deterministic routing + synonym intent map | **Implemented** | Application code maps recognised intents (Pearson metrics, sample-size synonyms, tract-count / not-patient clarifications) to JSON key paths or preferred documented passages, bypassing weak lexical scores. |
